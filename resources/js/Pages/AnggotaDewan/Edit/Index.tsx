...
export default function Edit({anggota_dewans} : {
    anggota_dewans: AnggotaDewan,
}) {
    const { data, setData, put, processing, reset , errors} = useForm({
        nama: anggota_dewans.nama,
        posisi: anggota_dewans.posisi,
    });

    const handlerSimpan = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('anggotadewan.update',), {
            onFinish: () => reset(),
        });
    }
    
    return (
        <Template title="Edit Anggota Dewan">
