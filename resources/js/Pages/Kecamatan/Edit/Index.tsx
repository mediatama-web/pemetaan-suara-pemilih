...
export default function Edit({kecamatan}: { kecamatan: { id: number, nama_kecamatan: string } }) {
    ...
    const handlerSimpan = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('kecamatan.update', data.id), {
            onFinish: () => reset('nama_kecamatan'),
        });
    }
    return (
        <Template title="Edit Kecamatan">
            <div className="container mx-auto p-3">
                ...
                <form onSubmit={handlerSimpan} className="bg-white p-6 rounded-lg shadow-md">
                    <div className="mb-4">
                        ...
                        <Input type="text" value={data.nama_kecamatan} onChange={(e) => setData('nama_kecamatan', e.target.value)} placeholder="Nama Kecamatan" name="nama_kecamatan" className={`${errors.nama_kecamatan ? 'border-red-600' : ''} mt-1 block w-full border rounded-md shadow-sm focus:ring focus:ring-opacity-50`} />
